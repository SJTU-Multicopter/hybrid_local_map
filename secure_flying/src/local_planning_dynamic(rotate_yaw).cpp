...
//            motion_primitives(p0, v0, a0, yaw_init, cost(seq,1), cost(seq,2), p_goal, cost(seq,3), v_max, SEND_DURATION, p, v, a, t);
        ...
        if(!collision_check_pass_flag){  // Safety mode
            ROS_WARN_THROTTLE(1, "No valid trajectory found! Trapped in safety mode!");
...
void setPointSendCallback(){
    ...
    if(safe_trajectory_avaliable){
        ...
        if(if_sim_lee_position_controller && if_in_simulation){
            buffer_send_counter_v = 4;
            buffer_send_counter_p = 4;
        }
...
void randomGoalGenerate()
{
    ...
    if(goal_generated_times > 10){
        for(int i=0; i<100; i++){
            ROS_ERROR("Test finished! Please shutdown");
        }
    }
}
...
void simPositionVelocityCallback(const nav_msgs::Odometry &msg)
{
    ...
    static double velocity_accumulation = 0.0;
    if(flight_started){
        velocity_accumulation += v0.norm();
        counter_times += 1;
        ROS_WARN_THROTTLE(1, "Average speed is: %f m/s, Flight time is: %f s", velocity_accumulation/counter_times, ros::Time::now().toSec()-started_time);
    }
}

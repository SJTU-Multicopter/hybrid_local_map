...
void cloudCallback(const control_msgs::JointControllerStateConstPtr &motor_msg, const sensor_msgs::PointCloud2ConstPtr& cloud, const nav_msgs::OdometryConstPtr &odom)
{

    simPositionVelocityCallback(*odom);
    ...
    std::cout << "cloud mismatch time = " << cloud->header.stamp.toSec() - collected_time << std::endl;
...
{
    ros::init(argc, argv, "local_planning_syn");
    ros::NodeHandle nh;
...
//        cloud_sub = new message_filters::Subscriber<sensor_msgs::PointCloud2>(nh, "/camera/depth/color/points", 2);
//
...
//        sim_odom_isolate_sub = nh.subscribe("/iris/ground_truth/odometry", 1, simPositionVelocityCallback);
        ...
        typedef message_filters::sync_policies::ApproximateTime<control_msgs::JointControllerState,sensor_msgs::PointCloud2,nav_msgs::Odometry> mapSyncPolicy;
        message_filters::Subscriber<control_msgs::JointControllerState> *motor_sub;
        message_filters::Subscriber<sensor_msgs::PointCloud2> *cloud_sub;
        message_filters::Subscriber<nav_msgs::Odometry> *pose_sub;

        motor_sub = new message_filters::Subscriber<control_msgs::JointControllerState>(nh, "/iris/joint1_position_controller/state", 5);
        cloud_sub = new message_filters::Subscriber<sensor_msgs::PointCloud2>(nh, "/iris/vi_sensor/camera_depth/depth/points", 5);
        pose_sub = new message_filters::Subscriber<nav_msgs::Odometry>(nh, "/iris/ground_truth/odometry", 100);

        message_filters::Synchronizer<mapSyncPolicy>* sync_;
        sync_ = new message_filters::Synchronizer<mapSyncPolicy>(mapSyncPolicy(20), *motor_sub, *cloud_sub, *pose_sub);
        sync_->registerCallback(boost::bind(&cloudCallback, _1, _2, _3));

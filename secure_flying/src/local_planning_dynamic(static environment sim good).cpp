...
void trajectoryCallback(const ros::TimerEvent& e) {
    ...
    bool collision_check_pass_flag = true;
    ...
    if(!state_updating) /// Need to rethink the necessity of this lock!!!! CHG
    {
        ...
        for(int seq=0; seq<pp.max_plan_num; seq ++)
        {
            ...
            static float init_state_change_square_thresh = 0.25;

            if((send_traj_buffer_p[0] - p0).squaredNorm() < init_state_change_square_thresh){
                ...
                motion_primitives(send_traj_buffer_p[0]*0.1 + p0*0.9, send_traj_buffer_v[0]*0.1+v0*0.9, a0, yaw_init, cost(seq,1),
                    cost(seq,2), p_goal, cost(seq,3), v_max, SEND_DURATION, p, v, a, t);
...
//            motion_primitives(p0, v0, a0, yaw_init, cost(seq,1), cost(seq,2), p_goal, cost(seq,3), v_max, SEND_DURATION, p, v, a, t);
    ...
    if(first_head_control_flag){
        ...
        if(collision_check_pass_flag) coefficient_planned_dir = 1.0 - _direction_update_buffer(getHeadingSeq(theta_h_chosen));
        ...
        for(int i=0; i<LOOKING_PIECES_SIZE; i++){
            ...
            double cost_current_velocity;
            if(v_too_small){
                cost_current_velocity = k_current_v * ((double)rand() / RAND_MAX) * 1.0;
            }else{
                cost_current_velocity = k_current_v * (v_direction-head_yaw_plan_temp) * (v_direction-head_yaw_plan_temp) * coefficient_current_v;
            }
...
void emergencyStop(bool &init, Eigen::Vector3d p0_local, Eigen::Vector3d v0_local)
{
	...
	if(!if_in_simulation){
    trackVelocityPose(sp_vx, sp_vy, sp_vz, 0.f, start_pose(0), start_pose(1), start_pose(2), yaw_init);
    }else{
	    if(if_sim_lee_position_controller){
            simTrackPose(start_pose(0), start_pose(1), start_pose(2), yaw_init);
        }else{
            trackVelocityPose(sp_vx, sp_vy, sp_vz, 0.f, start_pose(0), start_pose(1), start_pose(2), yaw_init);
        }
    }
...
//
//    /** Set a minimum height in case trapped in safety mode by ground **/
//    if(p0(2) < pp.collision_threshold_static + resolution)  p0(2) = pp.collision_threshold_static + resolution*2;
//
//    if(!offboard_ready)
//    {
//        trackVelocityPose(0.f, 0.f, 0.f, 0.f, p0(0), p0(1), p0(2), yaw_init);
...
//
//            float z_set;
//            if(if_plan_vertical_path) z_set = send_traj_buffer_p[buffer_send_counter](2);
//            else z_set = p_goal(2);
//
//            if(!if_in_simulation){ //Real UAV
...
//            }else{ // Rotors simulation
//                if(if_sim_lee_position_controller){
//                    simTrackPose(send_traj_buffer_p[buffer_send_counter+10](0), send_traj_buffer_p[buffer_send_counter+10](1), z_set, yaw_init);
//                }else{
//                    trackVelocityPose(send_traj_buffer_v[buffer_send_counter](0), send_traj_buffer_v[buffer_send_counter](1),
//                                      send_traj_buffer_v[buffer_send_counter](2), 0.f, send_traj_buffer_p[buffer_send_counter](0),
//                                      send_traj_buffer_p[buffer_send_counter](1), z_set, yaw_init);
...
//            if(fabs(p0(0)) > 5.f || fabs(p0(1)) > 5.f || fabs(p0(2)) > 5.0f) //very large, never use here
//            {
...
void randomGoalGenerate()
{
    ...
    p_goal(0) = x_distribution(random_generator);
    p_goal(2) = z_distribution(random_generator);
...
void simPositionVelocityCallback(const nav_msgs::Odometry &msg)
{
    ...
    if(fabs(p0(1))>FENCE_LEFT_SIZE && p0(1)*p_goal(1) > 0){
        randomGoalGenerate();
    }
}
...
//    p_goal << -1.0, -5.0, 1.2;
    ...
    if(!if_in_simulation){
        ...
        motor_sub = nh.subscribe("/iris/joint1_position_controller/state", 1, simMotorCallback);
        cloud_sub = nh.subscribe("/iris/vi_sensor/camera_depth/depth/points", 1, cloudCallback);

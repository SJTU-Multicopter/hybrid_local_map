...
const std::vector<std::vector<float> > all_points;
for (double (&pt)[2] : points)
{
    all_points.push_back(pt);
}

// all_points.push_back({0,0});
// all_points.push_back({1,1});
// all_points.push_back({2,2});
// all_points.push_back({3,3});
// all_points.push_back({4,4});
...
{ 
    for (int i=0; i<all_points.size(); ++i){
        cout<<all_points[i][0]<<","<<all_points[i][0]<<endl;
    }
